...
<div class="container">
    ...
    <div class="row">
        ...
        <c:if test="${empty products}">
            <div class="col-12 text-center">Không có sản phẩm khuyến mãi.</div>
        </c:if>
    </div>
        ...
        <nav aria-label="Page navigation">
            <ul class="pagination justify-content-center">
                <li class="page-item ${currentPage == 1 ? 'disabled' : ''}">
                    <a class="page-link" href="<c:url value='/promotions'><c:param name='page' value='${currentPage - 1}'/></c:url>">&laquo;</a>
                </li>
                <c:forEach begin="1" end="${totalPages}" var="i">
                    <li class="page-item ${currentPage == i ? 'active' : ''}">
                        <a class="page-link" href="<c:url value='/promotions'><c:param name='page' value='${i}'/></c:url>">${i}</a>
                    </li>
                </c:forEach>
                <li class="page-item ${currentPage == totalPages ? 'disabled' : ''}">
                    <a class="page-link" href="<c:url value='/promotions'><c:param name='page' value='${currentPage + 1}'/></c:url>">&raquo;</a>
                </li>
            </ul>
        </nav>
    </c:if>
